<?= card_open(isset($row)
    ? '<i class="icon cil-window"></i> Edit Customers'
    : '<i class="icon cil-window"></i> Tambah Customers') ?>
...
<form method="post">
    ...
    <?= bs_floating_input(
        'kd_cust',
        'date',
        isset($row) ? $row->kd_cust : '',
        'Tanggal PO',
        null,
        ['data-coreui-datepicker' => 'true']
    ) ?>

    <?= bs_floating_input('nm_cust', 'text', isset($row) ? $row->nm_cust : '') ?>
    <?= bs_floating_input('alamat1', 'text', isset($row) ? $row->alamat1 : '') ?>
    <?= bs_floating_input('alamat2', 'text', isset($row) ? $row->alamat2 : '') ?>
    <?= bs_floating_input('kota', 'text', isset($row) ? $row->kota : '') ?>
    <?= bs_floating_input('telepon', 'text', isset($row) ? $row->telepon : '') ?>

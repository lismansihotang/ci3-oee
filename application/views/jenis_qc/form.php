<?= card_open(isset($row) ? '<i class="icon cil-window"></i> Edit Jenis_qc' : '<i class="icon cil-window"></i> Tambah Jenis_qc') ?>
    <form method="post">
        <?= bs_floating_input('kd_qc', 'text', (isset($row) ? $row->kd_qc : '')); ?><?= bs_floating_input('kd_ms', 'text', (isset($row) ? $row->kd_ms : '')); ?><?= bs_floating_input('nama_qc', 'text', (isset($row) ? $row->nama_qc : '')); ?><?= bs_floating_input('satuan', 'text', (isset($row) ? $row->satuan : '')); ?>
        <div class="mt-3">
